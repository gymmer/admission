...
jQuery(function ($) {
     ...
     * 导航条悬停动画
     */
    var hideMenuLi = function hideMenuLi($li) {
        $li.removeClass('open').find('.dropdown>a').attr('aria-expanded', 'false');
    };

    var showMenuLi = function showMenuLi($li) {
        $li.addClass('open').find('.dropdown>a').attr('aria-expanded', 'true');
    };
    ...
    $('#banner-expand-button').click(function () {
        if ($(this).attr('data-expand') === 'false') {
            // 去展开
            $(this).attr('data-expand', 'true').html('<i class="fa fa-chevron-up"></i>收起');
            $('#index-banner-img-container').addClass('expand');
        } else {
            // 去收起
            $(this).attr('data-expand', 'false').html('<i class="fa fa-chevron-down"></i>展开');
